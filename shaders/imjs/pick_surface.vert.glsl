...
uniform uint u_materialExceeds;
vec4 g_materialParams;
...
float unpackFloat(vec4 v) {
  ...
  float unpacked =
      dot(sign * v.xyz,
          vec3(1.0 / 256.0, 1.0 / 65536.0,
               1.0 / 16777216.0)); // shift x right 8, y right 16 and z right 24
  return unpacked * pow(10.0, exponent);
}

void readMaterialAtlas() {
  float materialAtlasStart = u_vertParams.z * u_vertParams.w + u_numColors;
  float materialIndex = materialAtlasStart;
  if (u_materialExceeds > 0u) {
    vec2 tc = g_vertexBaseCoords;
    tc.x += (u_vertParams.z - 1.0) * g_vert_stepX;
    vec4 enc = floor(TEXTURE(u_vertLUT, tc) * 255.0 + 0.5);
    materialIndex += decodeUInt24(enc.xyz) * 4.0;
  } else {
    materialIndex += g_featureAndMaterialIndex.w * 4.0;
  }
  vec2 tc =
      computeLUTCoords(materialIndex, u_vertParams.xy, g_vert_center, 1.0);
  vec4 rgba = TEXTURE(u_vertLUT, tc);

  tc = computeLUTCoords(materialIndex + 1.0, u_vertParams.xy, g_vert_center,
                        1.0);
  vec4 weightsAndFlags = floor(TEXTURE(u_vertLUT, tc) * 255.0 + 0.5);

  tc = computeLUTCoords(materialIndex + 2.0, u_vertParams.xy, g_vert_center,
                        1.0);
  vec3 specularRgb = floor(TEXTURE(u_vertLUT, tc) * 255.0 + 0.5).rgb;

  tc = computeLUTCoords(materialIndex + 3.0, u_vertParams.xy, g_vert_center,
                        1.0);
  ...
  g_materialParams.y = weightsAndFlags.x + specularRgb.r * 256.0;
  g_materialParams.z = specularRgb.g + specularRgb.b * 256.0;
  g_materialParams.w = specularExponent;
}

vec4 getMaterialParams() { return g_materialParams; }
...
void computeMaterial() { readMaterialAtlas(); }
