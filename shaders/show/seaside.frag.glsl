...
float terrainIq(
    in vec2 x) // from IQ's Elevated : https://www.shadertoy.com/view/MdX3Rr
{
  vec2 p = x;
  float a = 0.0;
  float b = 1.0;
...
vec3 normalRocks(in vec2 p) {
  ...
  return normalize(vec3(rocks(p + e.xy) - rocks(p - e.xy), .008,
...
vec3 normalSea(in vec2 p) {
  ...
  return normalize(
      vec3(sea(p + e.xy) - sea(p - e.xy), .004, sea(p + e.yx) - sea(p - e.yx)));
}
