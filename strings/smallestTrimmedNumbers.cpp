/*
ou are given a 0-indexed array of strings nums, where each string is of equal
...
until only x digits remain. Strings in nums may contain leading zeros.
...
is 4 at index 3. There are two occurrences of 4, but the one at index 0 is
considered smaller than the one at index 3.
...
vector<int> smallestTrimmedNumbers(const vector<string> &nums,
  ...
  int n = queries.size();
  vector<vector<int>> cq(n);
  for (int i = 0; i < n; i++) {
    cq[i] = vector<int>{queries[i][1], queries[i][0], i};
  }
  sort(cq.begin(), cq.end());
  int m = nums.size(), l = nums[0].length();
  vector<int> vec(m), ans(n);
  iota(vec.begin(), vec.end(), 0);
  for (int j = 0, i = 1, k = 0; j < n; i++) {
    while (i < n && cq[i][0] == cq[j][0]) {
      i++;
    }
    for (; k < cq[j][0]; k++) {
      ...
      vector<int> cpy(m);
      for (int x = m - 1; x >= 0; x--) {
        cpy[count[nums[vec[x]][l - 1 - k] - '0'] - 1] = vec[x];
        count[nums[vec[x]][l - 1 - k] - '0']--;
      }
      for (int x = 0; x < m; x++) {
        vec[x] = cpy[x];
      }
...
int main() {
  auto r = smallestTrimmedNumbers({"102", "473", "251", "814"},
                                  {{1, 1}, {2, 3}, {4, 2}, {1, 2}});
  for (int i : r) {
    cout << i << " ";
  }
  cout << endl;
  r = smallestTrimmedNumbers({"24", "37", "96", "04"}, {{2, 1}, {2, 2}});
  for (int i : r) {
    cout << i << " ";
  }
  cout << endl;
  r = smallestTrimmedNumbers({"24", "37", "36", "04"}, {{2, 1}, {3, 2}});
  for (int i : r) {
    cout << i << " ";
  }
  cout << endl;
