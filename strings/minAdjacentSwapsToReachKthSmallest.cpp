...
int getMinSwaps(const string &num, int k) {
  string t(num);
  while (k--)
  ...
  for (int i = 0; i < n; ++i) {
    ...
    for (int j = i + 1; j < n; ++j) {
      if (num[i] != t[j])
        continue;
      // find the nearest num[i] at j in t
      ans += j - i;
      // remove t[j] and move substr [i,j) of t backwards by one char
...
int main() {
  cout << getMinSwaps("5489355142", 4) << endl;
  return 0;
}