/*
Given an integer array and a number N. Find if there are three integers inside
the array whose sum equals N.
*/
...
bool isPossible(int *arr, int siz, int N) {
  ...
  for (int i = lessSiz - 1; i >= 0; i--) {
    ...
    for (int j = base; j < greaterSiz; j++) {
      ...
      if (a > greater[j])
        continue;
      if ((a == v && m[a - min] > 1) || (a == greater[j] && m[a - min] > 1) ||
          (a != v && a != greater[j] && m[a - min] > 0)) {
