...
uint8_t reverseChar(uint8_t i) {
  uint8_t mask1 = 0x1, mask2 = 0x80, c = i;
  for (int i = 0; i < 4; i++) {
    if ((0 != (c & mask1)) != (0 != (c & mask2))) {
      c ^= mask1;
      c ^= mask2;
    }
    mask1 <<= 1;
...
int main() {
  cout << reverseBits(0b00000010100101000001111010011100) << endl;
  cout << reverseBits(0b11111111111111111111111111111101) << endl;
  return 0;
}