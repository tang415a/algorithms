...
// when forming the set for each query, if the lower bound of interval begins to exceed the query, break;
// the interval filtered out by the last query (because its higher bound is too small) in the following step remains filtered out by this query (its higher bound is even smaller).
...
// if the interval begins to cover the query, i.e. higher bound exceeds the query, bingo.
