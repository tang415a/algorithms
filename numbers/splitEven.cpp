/*
Write a function to determine if an array of non-negative numbers can be splited
evenly. Example 1: [1,3] false
...
bool possibleSum(int *arr, int siz, int start, int sum) {
  ...
  return (sum > arr[start] &&
          possibleSum(arr, siz, start + 1, sum - arr[start])) ||
         possibleSum(arr, siz, start + 1, sum);
}
