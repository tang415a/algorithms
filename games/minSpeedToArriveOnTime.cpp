...
mark. Return the minimum positive integer speed (in kilometers per hour) that
...
Output: -1
Explanation: It is impossible because the earliest the third train can depart is
at the 2 hour mark.
...
bool arriveOnTime(const vector<int> &dist, int speed, double hour) {
  double t = 0.0;
  int i = 0;
  for (int n = dist.size(); i < n - 1; i++) {
    t += (dist[i] + speed - 1) / speed;
  }
  return (t + static_cast<double>(dist[i]) / speed) <= hour;
}
...
int minSpeedOnTime(const vector<int> &dist, double hour) {
  ...
  if (hour <= n - 1)
    return -1;
  int l = 1, r = max(*max_element(dist.begin(), dist.end()),
                     static_cast<int>(dist[n - 1] / (hour - n + 1)) + 1);
  while (l <= r) {
    int mid = l + (r - l) / 2;
...
int main() {
  ...
  cout << minSpeedOnTime({1, 3, 2}, 2.01) << endl;
  return 0;
}