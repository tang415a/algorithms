...
than once. The last friend you counted leaves the circle and loses the game. If
there is still more than one friend in the circle, go back to step 2 starting
...
int findTheWinnerHelper(int n, int k) {
  if (n == 1)
    return 0;
  return ((k + 1) % n + findTheWinnerHelper(n - 1, k)) % n;
}

int findTheWinner(int n, int k) { return findTheWinnerHelper(n, k - 1) + 1; }

int findTheWinnerHelper2(int n, int k) {
  if (n == 1)
    return 0;
  return (k % n + findTheWinnerHelper2(n - 1, k)) % n;
}

int findTheWinner2(int n, int k) { return findTheWinnerHelper2(n, k) + 1; }

int main() {
  cout << findTheWinner(5, 2) << endl;
  cout << findTheWinner(6, 5) << endl;
  cout << findTheWinner2(5, 2) << endl;
  cout << findTheWinner2(6, 5) << endl;
  return 0;
}